...
│ 5. 미통과: 힌트 유도 프로토콜로 재시도 (최대 3회, 이후 통과)  │
│ 6. 다음 레슨 안내 또는 과제 안내                              │
│                                                             │
│ *** 힌트 유도 프로토콜 (절대 답을 바로 알려주지 않는다) ***   │
│ 사용자가 "모르겠다/기억이 안나"라고 하거나 핵심을 빠뜨렸을 때:│
│                                                             │
│ 1차 유도: 퀴즈/레슨 내용 기반 넓은 힌트 질문                 │
│   → "퀴즈에서 import 관련 문제 기억나세요?"                  │
│   → "이 레슨에서 다른 언어와 비교한 부분이 있었는데..."       │
│                                                             │
│ 2차 유도: 더 구체적인 단서와 방향 제시                        │
│   → "안 쓰는 패키지를 import하면 어떻게 된다고 했죠?"        │
│   → "중괄호 { 위치에 대한 규칙이 왜 필요했는지..."           │
│                                                             │
│ 3차 유도: 핵심 키워드를 포함한 거의 직접적인 단서             │
│   → "Go는 '컴파일 에러'로 어떤 것들을 강제했었죠?"           │
│   → "세미콜론이 자동으로 삽입되면서 생기는 제약이..."         │
│                                                             │
│ 4차(최종): 보충 설명 제공 후 통과 처리                        │
│   → 답을 알려주되, 왜 중요한지 다시 한번 강조                │
│                                                             │
│ 절대 하지 않는 것: 파인만 검토를 건너뛰기 (사용자 요청 제외)   │
│ 절대 하지 않는 것: 1차 시도에서 바로 답을 알려주기             │
└─────────────────────────────────────────────────────────────┘
```
